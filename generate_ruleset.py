...
for cdn_name, info in data.items():
    ...
    if ips:
        ...
        # Generate sing-box source JSON
        srs_json_filename = f"{safe_name}_ipcidr.json"
        srs_json_filepath = os.path.join(output_dir, srs_json_filename)
        singbox_ruleset = {
            "version": 1,
            "rules": [
                {
        ...
        with open(srs_json_filepath, "w", encoding="utf-8") as f:
            json.dump(singbox_ruleset, f, indent=2)
            
        srs_filename = f"{safe_name}_ipcidr.srs"
        srs_filepath = os.path.join(output_dir, srs_filename)
        # Compile it
        import subprocess
        subprocess.run(["sing-box", "rule-set", "compile", srs_json_filepath, "-o", srs_filepath], check=True)
        # Remove JSON to keep repo clean
        os.remove(srs_json_filepath)
        srs_path = f"ruleset/{srs_filename}"
...
for item in sorted(generated_files, key=lambda x: x["cdn"]):
    ...
    if item.get("srs"):
        srs_url = f"https://raw.githubusercontent.com/{github_repo}/main/{item['srs']}"
        srs_link = f"[{item['srs'].split('/')[-1]}]({srs_url})"
    else:
        srs_link = "-"
        
    readme_content += f"| {cdn} | {counts} | {ipcidr_link} | {classical_link} | {srs_link} |\n"
    ...
    "rule_set": [
      {
        ...
        "type": "remote",
        "format": "binary",
        "url": "https://raw.githubusercontent.com/{github_repo}/main/ruleset/cloudflare_ipcidr.srs",
